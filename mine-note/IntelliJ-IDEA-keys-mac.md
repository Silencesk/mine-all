...
* [参考](http://blog.csdn.net/rainytooo/article/details/51469126)
* 调试
````
alt+f8 debug时选中查看值  
...
````
 IntelliJ IDEA Mac下快捷键
标签： intellij ideamacide
2016-05-21 10:15 1165人阅读 评论(0) 收藏 举报
 分类： Vim IDE（1）  
目录(?)[+]
...
F12 跳到最后一个Tool Windows
⌘⌥T 包围代码（使用if..else, try..catch, for, synchronized等包围选中的代码）
⌘P 显示方法的参数信息
⌃⇧J 智能的将代码拼接成一行，针对dao层写的自定义sql很有效
⌘⇧U 大小写切换
⌘+ / ⌘- 展开 / 折叠代码块 折叠所有+shift
⌘⇧S 查询结构
⌘⌥← / ⌘⌥→ 退回 / 前进到上一个操作的地方
...
⌃← / ⌃→ 左右切换打开的编辑tab页
F12 返回到前一个工具窗口
⇧⎋ 隐藏当前或最后一个活动的窗口，且光标进入代码文件窗口 从工具窗口进入代码文件窗口
⌘⇧⌫ 跳转到最后一个编辑的地方
⌘B / ⌘ 鼠标点击 进入光标所在的方法/变量的接口或是定义处
⌘⌥B 跳转到实现处，在某个调用的方法名上使用会跳到具体的实现处，可以跳过接口
⌃⇧B 跳转到类型声明处
⌘U 前往当前光标所在方法的父类的方法 / 接口定义
⌃⇧P / ⌃⇧N ⌃↓ / ⌃↑ 当前光标跳转到当前文件的前一个/后一个方法名位置，自己设置
⌘] / ⌘[ 移动光标到当前所在代码的花括号开始/结束位置
⌃H 显示当前类的层次结构
⌘⇧H 显示方法层次结构
⌃⌥H 显示调用层次结构
