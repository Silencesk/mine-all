...
<#--页面标题-->
<#assign headerTitle = "单据列表demo模块">
<#--模块一定要传且不能和其他模块重复-->
<#assign modulePage="billpage">
...
<@override name="search-pane">
	<#--引入user-searchForm.ftl文件，该文件存放查询面板中输入框的集合	
		...
		注意：模板不存在就会报错
	-->
	<#include  "billpage-searchForm-layout.ftl" >
</@override>
<#--end: 覆盖重写 父模板 查询面板-->
...
<@override name="grid-pane">
	<#--引入user-searchForm.ftl文件，该文件存放查询面板中输入框的集合	
		...
		注意：模板不存在就会报错
	-->
	<#include  "billpage-grid-layout.ftl" >
</@override>
<#--end: 覆盖重写 父模板 网格区域-->
...
<@extends name="/ftl/common/pages/EydBillPage.ftl"/>