...
<#--页面标题-->
<#assign headerTitle = "blpo详情模块">
<#--模块一定要传且不能和其他模块重复-->
<#assign modulePage="blposubdtl">
...
<@override name="search-pane">
	<#--引入user-searchForm.ftl文件，该文件存放查询面板中输入框的集合	
		...
		注意：模板不存在就会报错
	-->
	<#include  "blposubdtl-editForm-layout.ftl" >
</@override>
<#--end: 覆盖重写 父模板 查询面板-->
...
<@override name="master-grid">
	<#--引入bassupplier-master-grid-layout.ftl文件，该文件存放查询面板中输入框的集合	
		...
		注意：模板不存在就会报错
	-->
	<#include  "blposubdtl-detailPart-layout.ftl" >
</@override>
<#--end: 覆盖重写 父模板 网格区域-->
...
<@override name="sub-grid">
	<#--引入bassupplier-sub-grid-layout.ftl文件，该文件存放查询面板中输入框的集合	
		...
		注意：模板不存在就会报错
	-->
	<#include  "blposubdtl-detailSubPart-layout.ftl" >
</@override>
<#--end: 覆盖重写 父模板 网格区域-->
